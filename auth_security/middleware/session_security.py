...
class SessionSecurityMiddleware:
    ...
    def __init__(self, get_response):
        ...
        self.inactivity_timeout = session_config.get('INACTIVITY_TIMEOUT', 3600)  # 1 hour
        ...
        if self.rotate_on_login:
            user_logged_in.connect(
                self._rotate_session_on_login,
                dispatch_uid='security_rotate_session'
            )
    ...
    def _check_session_validity(self, request: HttpRequest) -> tuple[bool, Optional[str]]:
        """
        ...
        if self.inactivity_timeout:
            ...
            if last_activity:
                inactive_duration = timezone.now() - timezone.datetime.fromisoformat(last_activity)
    ...
    def _rotate_session_on_login(self, sender, request, user, **kwargs):
        ...
        if hasattr(request, 'session'):
            ...
            if self.bind_to_user_agent:
                request.session['_security_user_agent'] = request.META.get('HTTP_USER_AGENT', '')
    ...
    if request.user.is_authenticated:
        ...
        if session_start:
            ...
            info.update({
                ...
                'time_until_absolute_timeout': max(0, absolute_timeout - int(session_age.total_seconds())),
            })
        ...
        if last_activity:
            ...
            info.update({
                ...
                'time_until_inactivity_timeout': max(0, inactivity_timeout - int(inactive_duration.total_seconds())),
            })
