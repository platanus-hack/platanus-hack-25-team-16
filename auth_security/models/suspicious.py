...
class SuspiciousLogin(models.Model):
    ...
    axes_attempt_id = models.IntegerField(
        null=True,
        blank=True,
        help_text=_("Related axes AccessAttempt ID")
    )
    ip_address = models.GenericIPAddressField(
        default='0.0.0.0',
        help_text=_("IP address of suspicious login")
    )
    ...
    timestamp = models.DateTimeField(
        auto_now_add=True,
        db_index=True,
        help_text=_("When suspicious login occurred")
    )
    ...
    notified = models.BooleanField(
        default=False,
        help_text=_("Whether user was notified about this login")
    )
    notified_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When user was notified")
    )
    reviewed = models.BooleanField(
        default=False,
        help_text=_("Whether this has been reviewed by security team")
    )
    reviewed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When this was reviewed")
    )
