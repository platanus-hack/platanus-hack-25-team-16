...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"  Risk Level: {report['executive_summary']['risk_level']}")
        self.stdout.write(
    ...
    def _print_summary(self, report):
        ...
        if "api_metrics" in report and report["api_metrics"]:
            ...
            if api["performance"]:
                self.stdout.write(
                    f"  Avg Response Time: {api['performance']['avg_response_time']:.2f}ms"
                )
        ...
        if "security_events" in report:
            ...
            self.stdout.write("\nSecurity Events:")
            self.stdout.write(
                f"  Failed Auth: {security['failed_authentications']['total']}"
            )
        ...
        self.stdout.write(
            "  Audit Logs: "
            + (
                self.style.SUCCESS("VALID")
                if integrity["audit_logs"]["status"] == "VALID"
        ...
        self.stdout.write(
            ...
            + (
                self.style.SUCCESS("VALID")
                if integrity["api_logs"]["status"] == "VALID"
                else self.style.ERROR("COMPROMISED")
            )
        )
