...
def load_backend(cfg: Dict[str, Any]) -> BaseAuditBackend:
    ...
    if not backend_path:
        raise ImproperlyConfigured("SECURITY_CONFIG.AUDIT_LOG.STORAGE_BACKEND no está definido.")
    try:
        backend_cls = import_string(backend_path)
    except Exception as exc:
        raise ImproperlyConfigured(f"No se pudo importar el backend {backend_path}") from exc
