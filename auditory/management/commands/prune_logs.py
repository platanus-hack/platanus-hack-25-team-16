...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--days", type=int, help="Días de retención; si no se indica usa la configuración.")
    ...
    def handle(self, *args, **options):
        ...
        retention = options.get("days") or cfg.get("AUDIT_LOG", {}).get("RETENTION_DAYS", 180)
        deleted = backend.prune(retention)
        self.stdout.write(self.style.SUCCESS(f"Eliminados {deleted} registros anteriores a {retention} días."))
