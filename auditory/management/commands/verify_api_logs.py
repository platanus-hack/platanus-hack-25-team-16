...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_logs == 0:
            self.stdout.write(
                self.style.WARNING("No API logs found to verify.")
            )
            return
        ...
        if result['ok']:
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ Hash chain integrity verified successfully!"
                )
            )
            self.stdout.write(
                self.style.SUCCESS(
    ...
    def _fix_chain(self, mismatches):
        ...
        for log in logs:
            ...
            content = f"{prev_hash}|{canonical_payload}|{log.timestamp.isoformat()}".encode("utf-8")
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Fixed {fixed_count} log entries.")
        )
    ...
    def _show_statistics(self):
        ...
        status_dist = APIRequestLog.objects.values('response_status').annotate(
        ...
        # Top endpoints
        endpoint_dist = APIRequestLog.objects.values('endpoint').annotate(
        ...
        auth_dist = APIRequestLog.objects.exclude(
        ...
        avg_time = APIRequestLog.objects.aggregate(
